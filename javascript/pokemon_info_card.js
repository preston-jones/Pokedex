...
async function loadPokemonInfoCard(i) {
...
// async function fetchNextPokemonInfoCard() {
...
function checkPokemonButton(i) {
    ...
    else if (i === loadedPokemonData.length - 1) {
        console.log('next:');
        document.getElementById(`next_pokemon_button_${i}`).classList.add("visibility_hidden");
    }
...
async function loadNextPokemonInfoCard(i) {
    let nextPokemon = i;
    if (i < loadedPokemonData.length - 1) {
        nextPokemon++;
        console.log('i:', nextPokemon);
        loadPokemonInfoCard(nextPokemon);
    }
    // else if (i === loadedPokemonData.length - 1) {
    //     await fetchNextPokemonInfoCard();
    //     loadPokemonInfoCard(loadedPokemonData.length - 1);
    // }
}
